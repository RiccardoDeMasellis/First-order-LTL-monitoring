...
public class FoLtlLocalForallFormula extends FoLtlQuantifiedFormula implements FoLtlLocalQuantifiedFormula,
	...
	public language.FormulaType getFormulaType(){
		return language.FormulaType.LOCAL_FORALL;
	}
	...
	public FoLtlLocalFormula quantifierExpansion(LinkedHashSet<FoLtlConstant> domain, FoLtlAssignment assignment){
		...
		while (i.hasNext()){
			...
			FoLtlLocalFormula temp = nested.quantifierExpansion(domain, assignment);

			if (res == null){
				res = temp;
			} else {
				res = new FoLtlLocalAndFormula(temp, res);
			}
